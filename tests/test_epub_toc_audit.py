...
def test_resolve_relative_href_handles_nested_directories():
    ...
    # sibling link
    assert (
        resolve_relative_href("Text/contents.xhtml", "ch01.xhtml") == "Text/ch01.xhtml"
    )
    # ../ link
    assert (
    ...
    assert (
        resolve_relative_href("Text/contents.xhtml", "ch01.xhtml#sec")
        == "Text/ch01.xhtml"
    )
    ...
    assert (
        resolve_relative_href("Text/toc.xhtml", "http://x/y.xhtml")
        == "http://x/y.xhtml"
    )
...
def test_relative_toc_href_in_nested_dirs_matches_extracted_spans(tmp_path):
    ...
    contents = epub.EpubHtml(
        title="Contents", file_name="Text/contents.xhtml", lang="en"
    )
    contents.content = (
...
def test_toc_document_start_boundaries_returns_stored_span_index():
    ...
    span_refs = [
        ...
        EpubSpanRef(
            span_index=5,
            block_id="b1",
            document_href="ch01.xhtml",
            spine_index=1,
            tag_name="p",
            source_text='<a href="ch01.xhtml">ONE</a>',
            source_text_sha256="h",
        ),
        EpubSpanRef(
            span_index=12,
            block_id="b2",
            document_href="ch02.xhtml",
            spine_index=2,
            tag_name="p",
            source_text='<a href="ch02.xhtml">TWO</a>',
            source_text_sha256="h",
        ),
    ]
...
def test_extract_writes_chapter_map_and_audit_report(tmp_path):
    ...
    assert (Path(project_dir) / ".booktx" / "reports" / "chapter-audit.json").is_file()
    # Warning-only preview findings: extract succeeded (exit 0 at _make_project
    # time) and produced both artifacts.
    report = json.loads(
        (Path(project_dir) / ".booktx" / "reports" / "chapter-audit.json").read_text(
            "utf-8"
        )
    )
